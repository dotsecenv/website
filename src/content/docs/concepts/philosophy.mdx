...
### Complete History Erasure

The append-only design means:

- Old entries remain readable to original recipients
- Revocation doesn't delete history
- Defrag can compact, but audit trail may be lost

This is a feature for compliance, not a bug.
...
```bash
# With git
git add vault.jsonl && git commit -m "Add API keys"
...
| UNIX philosophy | Do one thing, composable |
| Decentralized | GPG keys, no cloud required |
| Offline-first | Works without network |
| Development focus | SDLC, not production |
| Append-only | Audit trail, no data loss |

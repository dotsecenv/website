...
You likely already have a GPG key. GitHub and GitLab require one for [commit signature verification](https://docs.github.com/en/authentication/managing-commit-signature-verification/signing-commits). `dotsecenv` builds on that existing premise.
...
<Aside type="note">
  age is an excellent tool for its use case (simple file encryption). The lack
  of signing is not a criticism. It's a deliberate design choice by age's
  creators to keep the tool simple.
</Aside>
...
<Aside type="note">
  SOPS can use GPG alone, but the multi-identity workflow requires manual key
  management. dotsecenv provides `share` and `revoke` commands specifically for
  this.
</Aside>
...
See [Security Model](/concepts/security-model/) for details.

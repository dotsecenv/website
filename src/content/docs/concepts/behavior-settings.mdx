...
<Aside type="note">
  On shared hosts and CI, an administrator can pin any of these settings system-wide via the [policy directory](/concepts/policy-directory/) at `/etc/dotsecenv/policy.d/`. Policy values override whatever the user wrote here, with a warning to stderr explaining what changed.
</Aside>
...
<Aside type="note">
  `gpg.program` can also be pinned by the [policy directory](/concepts/policy-directory/), which is useful when admins want to force a specific GPG binary across an entire fleet.
</Aside>

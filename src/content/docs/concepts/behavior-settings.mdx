...
<Aside type="note">
  Admins can pin any of these settings system-wide via the [policy directory](/concepts/policy-directory/) at `/etc/dotsecenv/policy.d/` (v0.6.0+). When a user value disagrees with policy, the policy value wins and dotsecenv prints a warning to stderr.
</Aside>
...
<Aside type="note">
  `gpg.program` can also be pinned by the [policy directory](/concepts/policy-directory/) (v0.6.0+) to force a specific GPG binary across a fleet.
</Aside>

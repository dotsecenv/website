...
- AI tools and automation cannot exfiltrate decrypted values

<Aside type="caution" title="Lightweight Protection">
  This flag raises the bar for automated attacks but is not a complete defense. An attacker with code execution could bypass dotsecenv and call GPG directly using your key fingerprint (discoverable from the vault).

For stronger protection, configure GPG to never cache your passphrase (`default-cache-ttl 0` and `max-cache-ttl 0` in `gpg-agent.conf`). This forces manual passphrase entry via pinentry for every decryption, which most automated tools cannot provide.

</Aside>

See [Behavior Settings](/docs/concepts/behavior-settings/#require_tty_for_decryption) for configuration details.
...
| Automated exfiltration | Partial    | `require_tty_for_decryption` + pinentry |

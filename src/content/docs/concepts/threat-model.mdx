...
```bash
git add vault.jsonl   # Safe—secrets are encrypted
...
### BoringCrypto (Linux Builds)

Linux release binaries are built with Go's BoringCrypto backend (`GOEXPERIMENT=boringcrypto`), which provides FIPS-validated cryptographic primitives:

- BoringCrypto replaces Go's standard crypto implementations with BoringSSL
- BoringSSL is a fork of OpenSSL maintained by Google, certified for FIPS 140-2/140-3
- Provides hardware-accelerated AES, SHA, and RSA/ECC operations
- macOS builds use standard Go cryptography (BoringCrypto not available)
...
dotsecenv implements secure memory practices:

| Technique | Purpose |
|-----------|---------|
| `mlock()` | Prevent swapping secret data to disk |
| Secure zeroing | Overwrite memory after use |
| Process isolation | GPG operations in separate processes |

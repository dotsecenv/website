...
3. **Add them to your vault**

   ```bash
   dotsecenv vault identity add TEAMMATE_FINGERPRINT
   ```

   Replace `TEAMMATE_FINGERPRINT` with their actual GPG fingerprint.
...
4. **Share specific secrets**
   ...
   Or share all secrets:

   ```bash
   dotsecenv secret share DATABASE_PASSWORD TEAMMATE_FINGERPRINT --all
   ```

5. **Send the vault file**
...
Add all identities first, then share:

```bash
dotsecenv vault identity add ALICE_FINGERPRINT
dotsecenv vault identity add BOB_FINGERPRINT

...
- Fix `GPG_KEY_URL` to use `key.asc` (matching the deployed filename on get.dotsecenv.com)
- Use `GITHUB_TOKEN` for GitHub API requests in install script to avoid rate limits in CI

### Other

- Remove deprecated strict mode
- Add community standards (CODE\_OF\_CONDUCT, CONTRIBUTING, SECURITY)
- Dependency updates: go-crypto, golang.org/x/term, golang.org/x/sys, goreleaser-action, harden-runner, ghaction-import-gpg, attest-build-provenance, setup-terraform, artifact actions

...
do not need the target's private key — only their public key in your GPG
keyring.
...
<Aside type="note" title="Available in v0.6.0+">
  Introduced in **v0.6.0**. Not available on earlier versions.
</Aside>

Both subcommands are standalone — they don't load the user config or open vaults, so they keep working when policy is broken (which is exactly when you need them).

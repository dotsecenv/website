...
Install and use dotsecenv in GitHub Actions workflows to securely access secrets during CI/CD pipelines.
...
3. **Configure initialization (optional)**
   ...
   ```yaml
         - uses: dotsecenv/dotsecenv@v0
           with:
             init-config: ''  # Run with defaults
   ```

   <Aside type="tip">
     Set `init-config` to an empty string `''` to run `dotsecenv init config` with defaults, or provide specific flags like `--strict` or `--no-gpg-program`.
   </Aside>
     ...
     build:
       ...
       steps:
         ...
         - name: Build with secrets
           run: |
             dotsecenv version
             # dotsecenv run -- your-build-command
   ```
...
<Aside type="note">
  Use `latest` (the default) to always get the most recent release, or specify a version like `v0.1.0` for stability.
</Aside>
...
<Tabs>
  <TabItem label="Skip (default)">
    Don't run initialization:

    ```yaml
    - uses: dotsecenv/dotsecenv@v0
      # init-config defaults to 'skip'
    ```
  </TabItem>
...
Windows is not currently supported. For unsupported platforms, use `build-from-source: true`.
